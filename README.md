...
5. Go to Patches > ASM > Special Process Effects. Go to the Effects Code tab, and press the plus button. Click "Import Code", and then select starter_setter.asm from the folder in this repo that matches your ROM's region.
...
6. Repeat the prior step, using set_frame_color.asm.
7. Go back to the Special Process Effects tab. Click "Add Special Process" twice, and then scroll down to the bottom. The last two numbers in the "Effect" column should be zeros. Replace them with the lowest numbers that aren't already in use. If you haven't added any special processes before, this should be 61 and 62.
	* If you've already added special processes to the ROM before following this guide, write down the two numbers at the bottom of the ID column (the leftmost column); you'll need these later on.
...
10. Staying in the script engine debugger (the window that was opened when you opened smt.ssb), search for m00a01a.ssb, and open it. Replace the contents of this file with the contents of [m00a01a-US.exps](https://github.com/Chesyon/StarterMenuTool/blob/main/m00a01a-US.exps) or [m00a01a-EU.exps](https://github.com/Chesyon/StarterMenuTool/blob/main/m00a01a-EU.exps) from the repository, depending on your region. Save.
